defmodule IntegrationBeeWeb.StudentLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      >
        ...
        <.input field={@form[:course]} type="text" label="Course" />
        <:actions>
  ...
  def update(%{student: student} = assigns, socket) do
    changeset = Competition.change_student(student)
  ...
  def handle_event("validate", %{"student" => student_params}, socket) do
    changeset =
      ...
      |> Competition.change_student(student_params)
  ...
  defp save_student(socket, :edit, student_params) do
    case Competition.update_student(socket.assigns.student, student_params) do
      {:ok, student} ->
  ...
  defp save_student(socket, :new, student_params) do
    case Competition.create_student(student_params) do
      {:ok, student} ->
        notify_parent({:saved, student})
